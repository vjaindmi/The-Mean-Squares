...
myvars <- c("x.ageg5yr", "htin4", "weight2", "sex", "drnkany5", "exerany2", "sleptim1", 
            "physhlth", "chckidny", "educa", "income2", "diabete3")
...
# Select educa column having 1 to 6

survey_data_subset <- subset (survey_data_subset, educa <= 6 & educa >= 1)


#-----------------------------------------------------income2---------------------------------------------------------#

# Select marital column having 1 to 6

survey_data_subset <- subset (survey_data_subset, income2 <= 8 & income2 >= 1)
...
survey_data_subset_non_diabetec_subset <- survey_data_subset_non_diabetec [sample(nrow(survey_data_subset_non_diabetec),  48096), ]
...
survey_data_subset$educa <- as.factor (survey_data_subset$educa)
survey_data_subset$income2 <- as.factor (survey_data_subset$income2)
...
colnames (survey_data_subset) <- c("Age (Years)", "Height (CM)", "Weight (Kg)", "Sex (M_1_F_2)", "Had alcohol in past 30 Days? (Yes_1_No_2)", 
                                   "Did exercise in past 30 days (Yes_1_No_2)", "SleepHours", "Number of days physical health not good in past 30 days?", 
                                   "Have kideney disease? (Yes_1_No_2)", "Education (No school_1_Grade8_2_Grade9_3_Grade12_4_College_5_Graduate_6)",
                                   "Income level", "diabete3")
